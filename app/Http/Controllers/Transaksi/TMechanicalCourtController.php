...
class TMechanicalCourtController extends Controller
{
    ...
    public function edit($id, $wasit) {
        ...
        $mechanicalCourt = TMechanicalCourt::where('id_t_match', '=', $id)->where('referee', '=', $wasit)->get()->toArray();
        $dataMC = [];
        foreach ($mechanicalCourt as $item) {
            $dataMC[$item['id_m_mechanical_court']] = number_format($item['nilai'], 0, "", "");
        }
        ...
        return view('transaksi.t-match.mechanical-court.edit', [
            'id' => $id,
            'wasit' => $wasit,
            'model' => $model,
            'event' => $event,
    ...
    public function update(Request $request, $id, $wasit) {
        ...
        if ($data) {
            ...
            # DELETE OLD ITEM
            TMechanicalCourt::where('id_t_match', '=', $id)->where('referee', '=', $wasit)->delete();
                ...
                if ($child) {
                    ...
                    foreach ($child as $subitem) {
                        ...
                        if (empty($request->index[$subitem['id']])) {
                            DB::rollBack();
                            Session::flash('error', 'Nilai belum lengkap, mohon lengkapi penilaian.');
                            return redirect(route('mechanical-court.edit', [$id, $wasit]))->withInput();
                        }

                        # INSERT MODEL CHILD
                        $val = $this->insertChildMC($id, $wasit, $subitem, $item, $request->index[$subitem['id']]);
                        if ($val == 500) {
                            DB::rollBack();
                    ...
                    # INSERT MODEL PARENT
                    $val = $this->insertParentMC($id, $wasit, $item, $sum, $avg, $hasil);
                    if ($val == 500) {
                        DB::rollBack();
                        Session::flash('error', 'Gagal simpan. Mohon ulangi update penilaian.');
                        return redirect(route('mechanical-court.edit', [$id, $wasit]))->withInput();
            ...
            $evaluation->mechanical_court = $akhir * ( 25 / 100 );
            ...
            DB::commit();
            Session::flash('success', 'Mechanical Court berhasil diubah.');
            return redirect()->route('t-match.show', $id);
        }
        return redirect()->route('t-match.show', $id);
    }
