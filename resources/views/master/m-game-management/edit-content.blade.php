<x-base-layout>
    <?php
    $title    = 'Edit Header';
    $header   = \App\Models\Master\MGameManagement::select(['id', 'nama'])->where('level', '=', 1)->whereNull('deletedon')->get()->toArray();
    $headerId = (old('header')) ? old('header') : $model->id_m_game_management;
    ?>
    ...
    <div class="card shadow-sm">
        ...
        <div class="card-body">
            <form method="post" action="{{ route('m-game-management.update-content', $model->id) }}">
                @csrf
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-select" data-placeholder="Pilih Header ..." data-allow-clear="true" id="header" name="header">
                                ...
                                @foreach($header as $item)
                                    <option value="{{ $item['id'] }}" {{ ($headerId == $item['id']) ? 'selected' : '';}}>{{ $item['nama'] }}</option>
                                @endforeach
                            </select>
